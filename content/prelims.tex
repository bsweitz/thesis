...
\Tnote{maybe some justification for why?}
...
For a fixed degree \Tnote{degree $\to$ integer?} $d$ to be understood from context and a polynomial $p$ of degree at most $d$, let $N = \binom{n+d-1}{d}$. We use $\tilde{p}$ for the element of $\R^{N}$ which is the vector of coefficients of $p$ up to degree $d$. We use $\mbasis{d}$ to denote the vector of polynomials such that $p(x) = \tilde{p} \cdot \mbasis{d}$.

If $p$ is a polynomial of degree at most $2d$, then we also use $\hat{p}$ for an element of $\R^{N \times N}$ such that $p(x) = \hat{p} \cdot \mbasis{d}(\mbasis{d})^T$. Since multiple entries of $\mbasis{d}(\mbasis{d})^T$ are equal, there are multiple choices for $\hat{p}$, \Tnote{but $\to$ so for concreteness} but we choose the one that evenly distributes the coefficient over the equal entries. \Tnote{maybe say it follows from the definition of PSDness you gave above} Then $p = \sum_i q_i^2$ for some polynomials $q_i$ if and only if $\hat{p} \in \psd{N}$. Now if $p$ is a polynomial, we use $\|p\|$ to denote the largest absolute value of a coefficient of $p$. If $\cP$ is a set of polynomials, then $\|\cP\| = \max_{p \in \cP} \|p\|$.

\section{Semidefinite Programming and Duality}
At the heart of this thesis is investigating the power of semidefinite programming in general, and the specific Sum-of-Squares SDP in particular \Tnote{sentence reads weird}.
...
The \emph{dual} of an SDP $(C, \{A_i, b_i\}_{i=1}^m)$ is the optimization problem (with variables $(y,S)$):
    \Tnote{maybe $\min_{y,S}$ to make it clear what is being optimized}
\begin{align*}
&\min b \cdot y \\
...
\Tnote{I think this is just called slater's condition, so you can cite that?}
...
\begin{definition}\label{def:grobner}
    Let $\succ$ be an ordering on monomials such that if $x_U \succ x_V$ then $x_Ux_W \succ x_Vx_W$ \Tnote{I am not sure this is standard notation for monomials, might want to define it}. We say that $\cP$ is a \emph{Gr\"obner Basis} for $\gen{\cP}$ (with respect to $\succ$) if, for every $r \in \gen{\cP}$, there exists a $p \in \cP$ such that the leading term of $r$ is divisble by the leading term of $p$.
\end{definition}
...
If we pick $\theta$ in order to perform binary search,\Tnote{``if we pick in order to'' awkward wording} we can find $\max r(x)$ very quickly. One way to try and certify nonnegative polynomials is to express them as sums of squares.
...
\Tnote{reword? ``Because our goal is to optimize over some set $S$,...''}
...
When we have access to two sets of polynomials $\cP$ and $\cQ$ such that $S = V(\cP) \cap H(\cQ)$, as in our main motivation \Tnote{motivation $\to$ context?} of polynomial optimization, we can define a certificate of nonnegativity:
...
\end{definition}
\Tnote{The rest of this section is very smooth, reads really well}
...
Let $\cP$ and $\cQ$ be sets of polynomials and $r(x)$ be a polynomial. Pick any $p^* \in \cP$. If there exists a linear functional $\phi: \R[x_1,\dots,x_n] \rightarrow \R$ such that
    \Tnote{consider putting parens $\deg \cdot \to \deg(\cdot)$}
\begin{enumerate}
...
\item[(5)] $|\phi[\lambda p^*]| \leq \delta \|\lambda\|$ for every $\lambda$ such that $\deg \lambda p^* \leq 2d$,
    \Tnote{maybe intuition for this property? some kind of Lipschitz thing?}
\end{enumerate}
...
\end{proof}
Strong duality actually implies that the converse of \prettyref{lem:prelim_dual_cert} is true as well (clever \Tnote{again the clever readers thing kind of offends me haha, even though i did notice---could just make it ``the reader''} readers will notice that $\phi$ is a feasible solution to the dual of the SDP that finds degree $d$ PC$_>$ proofs), but as we only need this direction in this thesis we omit the proof of the converse. In
\prettyref{cha:bit_complexity} we further explore the problem of finding PC$_>$ proofs of bounded norm. We present some of the first theory \Tnote{``some of first theory'' awkward} for finding proofs of bounded norm, and we paint a promising picture for many relevant sets $\cP$ and $\cQ$.
...
\end{example}
\begin{definition}
    For a problem $\cM = (\cS, \cF)$ and approximation guarantees $c,s$, the \emph{$(c,s)$-Slack Matrix} $M$ is an operator that takes as input an $\alpha \in \cS$ and an $f \in \cF$ such that $\max_{\alpha \in \cS} f(\alpha) \leq S(f)$ and returns $M(\alpha,f) = C(f) - f(\alpha)$. \Tnote{capital S is atypo?}
\end{definition}
The slack matrix encodes the \Tnote{the $\to$ some ?} combinatorial properties of $\cM$, and we will see in the next section that certain propreties of the slack matrix correspond to the existence of specific convex relaxations that $(c,s)$-approximately solve $\cM$. In particular, in this thesis we investigate the power of SDPs as a way to solve $\cM$. \Tnote{tie back to slack matrices? otherwise I'd omit the last sentence, it is confusing and you are about to do it in the next section anyway}

\section{SDP Relaxations for Optimization Problems}\label{prelims_sdp_relaxations}
A popular method for solving combinatorial optimization problems is to formulate them as SDPs, and use generic algorithms \Tnote{such as ellipsoid} for solving SDPs.
\begin{definition}
...
\Tnote{I suggest that you add some prose here that says you are going to make a connection between the existance of a slack matrix and the size of the SDP required, because it is an important moment}
...
&q(x) \geq 0, \forall q \in \cQ.
\end{align*}
Solving this optimization problem is equivalent to solving the combinatorial optimization \Tnote{problem} $\cM$.

In \prettyref{sec:polyproofs} we discussed how polynomial optimization problems could sometimes be solved by searching for PC$_>$ proofs of nonnegativity. Furthermore, these proofs can be found using semidefinite programming. It should come as no surprise then that, having rephrased $\cM$ as a polynomial optimization problem, there are SDP relaxations based on finding certificates of nonnegativity \Tnote{this is kind of wordy, but i dont have suggestions for fixing it}. The Theta Body relaxation, first considered in \cite{GPT10}, is defined only for formulations without inequality constraints. It finds a certificate of nonnegativity for $r(x)$ which is an SOS polynomial $s(x)$ together with a polynomial $g \in \gen{\cP}$ such that $r(x) = s(x) + g(x)$.

Let $(\cP, \cO, \phi)$ be a degree-$d$ polynomial formulation for $\cM$. Recall that every polynomial $p$ of degree at most $2d$ has a $d \times d$ matrix of coefficients $\widehat{p}$ such that $p(\alpha) = \widehat{p} \cdot \left( \mbasis{d}(\alpha)(\mbasis{d}(\alpha))^T\right)$.
\begin{definition}
For $D \geq d$, the \emph{$D$th Theta-Body Relaxation} of $(\cP,\cO,\phi)$ is an SDP relaxation for $\cM = (\cS, \cF)$ consisting of:
\begin{itemize}
    \item \emph{Semidefinite program:} $\widehat{p} \cdot X = 0$ for every $p \in \gen{\cP}$ of degree at most $2D$, $\widehat{1} \cdot X = 1$, and $X \succeq 0$ \Tnote{maybe recall that hat 1 is the matrix polynomial corresponding to 1}. For each polynomial $o^f \in \cO$, we define the affine function $w^f(X) = \widehat{o^f} \cdot X$.
\item Feasible solutions: $X^\alpha = \mbasis{D}(\phi(\alpha))(\mbasis{D}(\phi(\alpha)))^T$.
    \Tnote{what is $\phi$? what is $\alpha$?}
\end{itemize}
\end{definition}
...
&\widehat{s} \succeq 0
\end{align*}
    \Tnote{more canonical to do $c \cdot M$ when $c$ is a scalar and $M$ is a matrix, i think}
    The equality constraint of the dual is a constraint on matrices, but we can also think of it as a constraint on degree $2D$ polynomials via the map $\widehat{p} \leftrightarrow p$. Recall $\widehat{s} \succeq 0$, if and only if $s$ is a sum-of-squares polynomial \Tnote{of the proper degree... right?}. Thus this constraint is equivalent to asking that the polynomial $c - o^f(x)$ be $2D$-SOS modulo $\gen{\cP}$.
\end{proof}
...
The dual of the SOS relaxation is $\min y_1$ subject to $\hat{s} \succeq 0$ and
    \Tnote{I thought $\hat 1$ was a matrix, what does this diag mean? block diagonals? clarify pls}
\begin{align*}
...
\end{example}
\prettyref{prop:tb_to_sos} allows us to translate results about the Theta Body relaxations to Lasserre relaxations. In particular, in \prettyref{cha:symmetric_sdps} we will see how easy it is \Tnote{to} prove that Theta Body relaxations are optimal among symmetric relaxations of a given size. If the constraints are effective, this allows us to conclude that Lasserre relaxations which are not too much larger achieve the same guarantees. This allows us to lower bound the size of \emph{any} symmetric SDP relaxation by finding lower bounds for Lasserre relaxations.

\section{Symmetric Relaxations}
\Tnote{this whole section needs more examples---maybe you can use the same example problem a couple of times, but as is it is hard to wrap head around it}
Often, combinatorial optimization problems will have underlying symmetries in their solution spaces \Tnote{maybe instead say that the ''solutions universally exhibit symmetries''}. This extra structure allows us to prove things about these problems more easily. \Tnote{here there is a bit of a leap between symmetric solutions and symmetric formulations, maybe say why it makes sense to have a symmetric formulation for a symmetric problem} In this section we define what we mean by symmetric versions of all the problem formulations we have presented above. First, we recall some basic group theory.
\begin{definition}
...
An SDP relaxation $(\{X^\alpha\},\{(A_i,b_i)\},\{w^f\})$ for a $G$-symmetric problem $\cM$ is $G$-symmetric if there is an action of $G$ on $\psd{d}$ such that $gX^\alpha = X^{g\alpha}$ for every $\alpha$, and $w^{gf}(gX) = w^f(X)$, and $A_i \cdot X = b$ for all $i$ if and only if $A_i \cdot gX = b$ for all $i$. We say the relaxation is $G$-coordinate-symmetric if the action of $G$ is by permutation of the coordinates, in other words $G$ has an action on $[d]$ and $(gX)_{ij} = X_{gi,gj}$.
    \Tnote{give an example of a symmetric and an asymmetric SDP relaxation?}
\end{definition}
