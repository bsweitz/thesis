...
Gr\"obner bases are objects first considered in \cite{Buch65} as a way to determine if a polynomial $r$ is an element of $\gen{\cP}$. We define them here and include some of their important properties.
...
\begin{lemma}[Hilbert's Weak Nullstellensatz \cite{Hilbert1893}]\label{lem:nullstellensatz}
...
The following lemma is an easy but important fact, which we will use to construct derivations in \prettyref{cha:effective_derivations}.
...
The problem of finding a degree-$d$ HN derivation for $r$ can be expressed as a linear program with $n^d|\cP|$ variables, since the polynomial identity is linear in the coefficients of the $\lambda_i$. Thus if such a derivation exists, it is possible to find efficiently in time polynomial in $n^d|\cP|$, $\log \|\cP\|$ and $\log \|r\|$. These parameters are all polynomially related to the size required to specify the input: $(r,\cP,d)$.
\begin{definition}
...
Because our goal is to optimize over some set $S$, we actually only care about the non-negativity of a polynomial on $S$ rather than on all of $\R^n$.
...
\end{proof}
Strong duality actually implies that the converse of \prettyref{lem:prelim_dual_cert} is true as well (the reader might notice that $\phi$ is actually a hyperplane separating $r$ from the set of polynomials with bounded coefficients for $p^*$), but as we only need this direction in this thesis we omit the proof of the converse. Also note that if $\delta = 0$, i.e. $\phi$ satisfies $\phi[\lambda p^*] = 0$, then the lemma implies there are no proofs of non-negativity for $r$. 
%In \prettyref{cha:bit_complexity} we further explore the problem of finding PC$_>$ proofs of bounded norm. We present some of the first results for finding proofs of bounded norm, and we paint a promising picture for many relevant sets $\cP$ and $\cQ$.
...
\begin{theorem}[Generalization of Yannakakis' Factorization Theorem]\label{thm:yannakakis}
...
\section{Symmetric Relaxations}\label{sec:symmetric-defs}
