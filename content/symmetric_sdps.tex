...
Recall the polynomial formulation of \textsc{TSP} on $n^2$ variables from \prettyref{eq:tsp-formulation}. The map $\phi$ is defined so that for a tour $\tau$, $\phi(\tau) = \chi_\tau$, where $(\chi_\tau)_{ij} = 1$ if $\tau(i) = j$ and $0$ otherwise.
