...
\end{align}
We omit the dependence on $n$ when it is clear from context. For an element $\sigma$ of the symmetric group $\sym{n}$, we define the action of $\sigma$ on a variable by $\sigma x_{ij} = x_{\sigma(i)\sigma(j)}$.
...
\begin{align*}
Lx_{n+1,n+2} - \left(\sum_{j=1}^{n+2} x_{ij} - 1\right)x_{n+1,n+2} &= -x_{i,n+1}x_{n+1,n+2} - x_{i,n+2}x_{n+1,n+2} \\
&\cong_2 0.
\end{align*}
\end{proof}
