...
\begin{definition}
TODO Recall we use $\mbasis{d}$ denote the vector whose entries are all the monomials in $\R[x_1,\dots,x_n]$ up to total degree $d$. 
...
\[M_{S,d} = \E_{\alpha \in S}\left[\mbasis{d}(\alpha)\mbasis{d}(\alpha)^T\right]\]
\end{definition}
Clearly $M_{S,d}$ is a PSD matrix, and furthermore it encodes a lot of information about the set $S$. For example, if we let $c \in \R^{\binom{n+d-1}{d}}$, then $c$ corresponds to the polynomial $\tilde{c}(x) = c^T\mbasis{d}$, and then $c^TM_{S,d}c = \E_{\alpha \in S}\left[\tilde{c}(\alpha)^2\right]$.
...
\begin{definition}
Let $S \subseteq V(\cP)$. We say that $\cP$ is \emph{$k$-complete for $S$ up to degree $d$} if, for every zero eigenvector $c$ of $M_{S,d}$, the degree $d$ polynomial $\tilde{c}(x) = c^T\mbasis{d}$ has a derivation from $\cP$ in degree $k$.
\end{definition}
...
\begin{align*}
\E_{\alpha \in S}\left[r(\alpha)\right] &= \left\langle C, \E_{\alpha \in S}\left[\mbasis{d}(\alpha)\mbasis{d}(\alpha)^T\right]\right\rangle + \sum_{i=1}^\ell \left\langle D_i, \E_{\alpha \in S}\left[\mbasis{d}(\alpha)\mbasis{d}(\alpha)^T\right]\right\rangle q_i(\alpha) + 0 \\
\end{align*}
...
%
However, because $\cP$ is complete for $S$, these can be absorbed into the final term. More formally, let $\Pi = \sum_u uu^T$ be the projector onto the zero eigenspace of $M_{S,d} = \E_{\alpha \in S}[\mbasis{d}(\alpha)\mbasis{d}(\alpha)^T]$. Because $\cP$ is $k$-complete for $S$, for each $u$ there is a degree $k$ derivation $u^T\mbasis{d} = \sum_i \sigma_{ui} p_i$. Then $\Pi \mbasis{d}(\mbasis{d})^T = \sum_u (u^T\mbasis{d}) \cdot u(\mbasis{d})^T$. Thus we can write
\begin{align*}
