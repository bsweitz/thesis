...
\end{corollary}
\begin{proof}
	Recall $M_{S,d} = \E_{\alpha \in S}[\mbasis(\alpha)\mbasis(\alpha)^T]$, and note that $|S| \cdot M$ is an integer matrix with entries at most $2^n$.  The result follows by applying \prettyref{lem:integer}. 
\end{proof}
...
Let $\epsilon < 1/2$. Then any PC$_>$ proof of $\epsilon - x_1$ from $\cP$ of degree $d$ must involve polynomials of norm at least $\Omega(\frac{1}{n^d}\left(\frac{1}{2\epsilon}\right)^{2^n}$.
\end{lemma}
\begin{proof}
We will define a linear functional $\phi: \R[X]_d \rightarrow \R$ as in TODO: CITE PRELIMS. Recall we want $\phi$ to satisfy the following:
\begin{enumerate}
